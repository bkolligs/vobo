...
#define VOBO_DEBUG_LOG(...)                                          \
    std::cerr << ASCI_ESCAPE COLOR_GREEN << __VA_ARGS__ << END_COLOR \
              << std::endl
#define VOBO_WARNING_LOG(...)                                         \
    std::cerr << ASCI_ESCAPE COLOR_YELLOW << __VA_ARGS__ << END_COLOR \
              << std::endl
#define VOBO_ERROR_LOG(...) \
    std::cerr << ASCI_ESCAPE COLOR_RED << __VA_ARGS__ << END_COLOR << std::endl
#define VOBO_CRITICAL_LOG(...)                                       \
    std::cerr << ASCI_ESCAPE COLOR_RED_BOLD << __VA_ARGS__ END_COLOR \
              << std::endl

enum class DebugLevel { INFO = 0, DEBUG, WARNING, ERROR, CRITICAL };
